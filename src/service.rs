		...
		let result: Result<Box<[_]>, crate::Error> =
			...
			.map(|name| -> Result<_, crate::Error> {
				let monitor = match &*name {
					...
					"ddclient" => crate::config::ServiceMonitor::PidFile("/var/run/ddclient.pid".into()),
					...
					"haproxy" => crate::config::ServiceMonitor::PidFile("/var/run/haproxy.pid".into()),
					"nginx" => crate::config::ServiceMonitor::PidFile("/var/run/nginx.pid".into()),
					...
					"tor" => crate::config::ServiceMonitor::PidFile("/var/run/tor/tor.pid".into()),
